# 项目重构指南：后端架构升级 (Backend Refactoring)
...
建议在根目录下创建 `backend/` 目录，与 `frontend/` 并列。
...
- **前端零修改**: 后端重构不应导致前端 `frontend/src/api` 下的代码变更。
- **关键接口列表**:
  - `/api/login` (POST)
  - `/api/status` (GET)
  - `/api/automation` (POST)
...
- [ ] **项目初始化**: 创建 `backend` 目录，配置 NestJS + TypeScript + pnpm。
